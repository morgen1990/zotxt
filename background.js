...
function selectEndpoint(options) {
    let ZoteroPane = Zotero.getMainWindow().ZoteroPane;
    let promise = null;
    const easykey = options.searchParams.get('easykey');
...
async function startup({id, version, resourceURI, rootURI = resourceURI.spec}) {
    Zotero.debug('zotxt: Starting up version ' + version);
    // Core.js is already included in this file, no need to load it
    loadEndpoints(version);
}

function shutdown () {
    Zotero.debug('zotxt: Shutting down');
}
